...
class APIDownloadHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if not ajaxCheckAcess(self, readFile):
            return
        ...
        try:
            ...
            self.finish(
                json.dumps({"success": 1, "message": fr, "md5": md5code, "filename": os.path.basename(readFile)}))
        except:
...
class APIUploadHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if not ajaxCheckAcess(self, saveFile):
            return
        if os.path.exists(saveFile):
...
class APIDeleteHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if filename.startswith('/'):
            filename = '.' + filename
        deleteFile = os.path.join(FileStoragePath, filename)
        if not ajaxCheckAcess(self, deleteFile):
            return
        if not os.path.exists(deleteFile):
...
class APIListHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if not ajaxCheckAcess(self, path):
            return
...
class APIMD5Handler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if filename.startswith('/'):
            filename = '.' + filename
        md5file = os.path.join(FileStoragePath, filename)
        if not ajaxCheckAcess(self, md5file):
            return
        if not os.path.exists(md5file):
        ...
        if not os.path.isfile(md5file):
            ...
            return
        self.finish(
            json.dumps({"success": 1, "message": md5.new(open(md5file).read()).hexdigest()}))
...
class APIInfoHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if filename.startswith('/'):
            filename = '.' + filename
        infofile = os.path.join(FileStoragePath, filename)
        if not ajaxCheckAcess(self, infofile):
            return
        if not os.path.exists(infofile):
            self.finish(json.dumps({"success": 0, "message": u"文件不存在"}))
            return
